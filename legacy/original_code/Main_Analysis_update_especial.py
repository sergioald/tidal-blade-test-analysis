    ...
    if Type_Test=='ST':
        if DO_ST==1:
            ...
            #tdms_file = TdmsFile(os.path.join(f_path,f_fname+'.tdms'))
                ...
                for ld_ch in all_lds_lst:
                    len_d=int(min(all_loads_filt_com[ld_ch].shape[0],len_d))
                all_loads_filt_m=np.empty((len_d, len(all_lds_lst)))
                ...
                for dis in all_sen_lst:
                    print(dis)
                    if dis=='Pos_S_Tip':
                        disp=tdms_file['Log'][dis][:]+tdms_file['Log']['Position_A_03_Filter'][:tdms_file['Log'][dis][:].shape[0]]
                        # disp=disp-np.mean(zero_tdms_file['Log'][dis][:])
                        print('enter special')
                    elif dis=='Pos_S_Cent':
                        disp=tdms_file['Log']['Pos_S_Tip'][:]-np.mean(zero_tdms_file['Log'][dis][:])
                    else:    
        ...
        if DO_FA==1:
            ...
            for ffiles in ft_files:
                ...
                #tdms_file = TdmsFile(os.path.join(f_path,f_fname+'.tdms'))
                    ...
                    peaks_up, _ = find_peaks(rbm, prominence=np.mean(rbm))#(np.sum(dist_act*5)))#,distance=100)
                    peaks_dwn, _ = find_peaks(rbm*-1, prominence=np.mean(rbm))#(np.sum(dist_act*5)))#,distance=100)
                    ...
                    for dis in all_sen_lst:
                        ...
                        while cond<1:
                            if np.isnan(disp).all():
                                ...
                                # per_rbm=np.sum(dist_act*fct)/rbm.mean()
                                ...
                                disp = cycle_spin(disp, func=denoise_wavelet, max_shifts=s,
                                    func_kw=denoise_kwargs, channel_axis=-1)
                                per_rbm=fct
                                time_indx_max, _ = find_peaks(disp, prominence=per_rbm*mn)#),distance=100)
                                time_indx_min, _ = find_peaks(disp*-1,prominence=per_rbm*mn)#),distance=100)
                                try:
                        ...
                        # plt.plot(disp*(rbm.max()/disp.max()))
                        # plt.plot(time_indx_max,disp[time_indx_max]*(rbm.max()/disp.max()),'o')
                        # sdsa
                        # plt.close()
                        # dsad
                        # else:
