    ...
    if row['Type_Test']=='ST':
        if DO_ST==1:
            ...
            #tdms_file = TdmsFile(os.path.join(f_path,f_fname+'.tdms'))
                ...
                all_chn_lst=[key for key in tdms_file['Log']._channels if not key.__contains__("ilter") ]
                ...
                ch1='Str_Lin_120_1_1_L1'
                cht='Temp_S_T_06'
                strt=np.argmin(abs(tdms_file['Log'][ch1].time_track(absolute_time=True)-stc_start0))
                end=np.argmin(abs(tdms_file['Log'][ch1].time_track(absolute_time=True)-stc_end0))
                
                strng=(tdms_file['Log'][ch1][strt:end]-np.mean(zero_tdms_file['Log'][ch1][:]))
                strngt=tdms_file['Log'][ch1].time_track()[strt:end]
                strng=strng*-1
                        
                strt=np.argmin(abs(tdms_file['Log'][cht].time_track(absolute_time=True)-stc_start0))
                end=np.argmin(abs(tdms_file['Log'][cht].time_track(absolute_time=True)-stc_end0))        
                temp=tdms_file['Log'][cht][strt:end]
                tempt=tdms_file['Log'][cht].time_track()[strt:end]
                
                from scipy import signal
                
                
                rsmp=signal.resample(strng, len(temp), t=strngt)
                strng=rsmp[0]
                strngt=rsmp[1]
                
                sigma_est = estimate_sigma(strng, average_sigmas=True)
                strng=denoise_wavelet(strng, method='VisuShrink', mode='soft',  
                                       wavelet='sym20', 
                                       sigma=sigma_est*1,
                                       rescale_sigma='True')
                
                sigma_est = estimate_sigma(temp, average_sigmas=True)
                temp=denoise_wavelet(temp, method='VisuShrink', mode='soft',  
                                       wavelet='sym20', 
                                       sigma=sigma_est*1,
                                       rescale_sigma='True')
                
                peaks_ups, _ = find_peaks(strng, prominence=(0.01*np.min(strng)))
                peaks_dwns, _ = find_peaks(strng*-1, prominence=(0.01*np.min(strng)))
                
                peaks_upt, _ = find_peaks(temp, prominence=(0.01*np.max(temp)))
                peaks_dwnt, _ = find_peaks(temp*-1, prominence=(0.01*np.max(temp)))
                ...
                fig,ax=plt.subplots()
                ax.plot(strngt,strng)
                
                
                regr = linear_model.LinearRegression()
                regr.fit(strngt.reshape(-1, 1),strng.reshape(-1, 1))
                ax.plot(strngt,regr.predict(strngt.reshape(-1, 1)))
                print(regr.coef_)
                ax.plot(strngt[peaks_ups], strng[peaks_ups], "x",c='b')
                ax.plot(strngt[peaks_dwns], strng[peaks_dwns], "o",c='b')
                
                
                
                ax2=ax.twinx()
                ax2.plot(tempt,temp,'k')
                regr = linear_model.LinearRegression()
                regr.fit(tempt.reshape(-1, 1),temp.reshape(-1, 1))
                ax2.plot(tempt,regr.predict(tempt.reshape(-1, 1)),'k')
                print(regr.coef_)
                ax2.plot(tempt[peaks_upt], temp[peaks_upt], "x",c='k')
                ax2.plot(tempt[peaks_dwnt], temp[peaks_dwnt], "o",c='k')
                
                delta=400
                plt.figure()
                plt.plot(strng[delta:],temp[:-delta])
                
                fig,ax=plt.subplots()
                ax.plot(strngt-delta,strng)
                ax2=ax.twinx()
                ax2.plot(tempt,temp,'k')
