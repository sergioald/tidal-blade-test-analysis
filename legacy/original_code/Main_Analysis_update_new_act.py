    ...
    if Type_Test=='ST':
        if DO_ST==1:
            ...
            #tdms_file = TdmsFile(os.path.join(f_path,f_fname+'.tdms'))
                ...
                if len(all_lds_lst)>1:
                    rbm_all=all_loads_filt_m*dist_act*per_y
                ...
                targ_load=statistics.mode(rbm[rbm>100].astype(int))
                ...
                for ld_ch in all_lds_lst:
                    if all_loads_filt_com[ld_ch].shape[0]!=len_d:
                       atemp=all_loads_filt_com[ld_ch][:int(min(all_loads_filt_com[ld_ch].shape[0],len_d))] 
                       all_loads_filt_m[:,cnt_l]=atemp
        ...
        if DO_FA==1:
            ...
            for ffiles in ft_files:
                ...
                #tdms_file = TdmsFile(os.path.join(f_path,f_fname+'.tdms'))
                    ...
                    peaks_up, _ = find_peaks(rbm, prominence=(np.sum(dist_act*20)))#,distance=100)
                    peaks_dwn, _ = find_peaks(rbm*-1, prominence=(np.sum(dist_act*20)))#,distance=100)
                    ...
                    for dis in all_sen_lst:
                        ...
                        print(dis,np.nanmean(zero_tdms_file['Log'][dis][:]))
                        # try:
                        #     disp=tdms_file['Log'][dis+'_Filter'][:]-np.nanmean(zero_tdms_file['Log'][dis+'_Filter'][:])
                        # except:
                        #     5
                        ...
                        # plt.figure()
                        # plt.plot(rbm*(disp.max()/rbm.max()))
                        # plt.plot(peaks_up,rbm[peaks_up]*(disp.max()/rbm.max()),'o')
