        ...
        if DO_ST==1:
            ...
            #tdms_file = TdmsFile(os.path.join(f_path,f_fname+'.tdms'))
                ...
                if len(all_lds_lst)>1:
                    ...
                    rbm = np.sum(rbm_all, axis=1)
                    rbm[rbm<0]=0
                    rbm= np.sum(all_loads_filt_m, axis=1)
                    rbm[rbm<0]=0 
            ...
            for ffiles in ft_files:
                ...
                #tdms_file = TdmsFile(os.path.join(f_path,f_fname+'.tdms'))
                    ...
                    if len(all_lds_lst)==1:
                        ...
                        s=6
                        denoise_kwargs = dict( wavelet='sym19',
                                              sigma=sigma_est*filt_lev,
                                              rescale_sigma=True)
                        ...
                        for ld_ch in all_lds_lst:
                            ...
                            tot_load=(tdms_file['Log'][ld_ch][:])#-np.mean(zero_tdms_file['Log'][ld_ch][:]))
                            time_ld=tdms_file['Log'][ld_ch].time_track(absolute_time=True,accuracy='ms')
                            if np.mean(tot_load)<0:
                                tot_load=tot_load*-1
                    ...
                    for dis in all_sen_lst:
                        ...
                        s=10
                        denoise_kwargs = dict( wavelet='sym6',
                                              sigma=sigma_est*filt_lev,
                                              rescale_sigma=True)
                        ...
                        while cond<1:
                            if np.isnan(disp).all():
                                ...
                                per_rbm=np.sum(dist_act*fct)/rbm.max()
                                time_indx_max, _ = find_peaks(disp, prominence=per_rbm*disp.max())#),distance=100)
                                time_indx_min, _ = find_peaks(disp*-1,prominence=per_rbm*disp.max())#),distance=100)
                                try:
                                    if time_indx_max[0]<time_indx_min[0]:
                                        time_indx_min=np.insert(time_indx_min, 0,0)
                                    if time_indx_max[-1]>time_indx_min[-1]:
                                        time_indx_min=np.append(time_indx_min,disp.shape[0]-1)
                                except:
                                    time_indx_max=time_indx_max_old
                                    time_indx_min=time_indx_min_old
                                    print('Use old index')
                                    cond=2
                                if (len(time_indx_max)==len(peaks_up)) and (len(time_indx_min)==len(peaks_dwn)):
                                    cond=2
                                
                                else:
                                    # print(len(time_indx_max),len(peaks_up))
                                    fct=fct+0.1
                        ...
                        plt.figure()
                        disp.max()
                        plt.plot(time_ld,tot_load*(disp.max()/tot_load.max()))
                        plt.plot(time_ld[peaks_up],tot_load[peaks_up]*(disp.max()/tot_load.max()),'x')
                        
                        plt.plot(time_dis,disp)
                        plt.plot(time_dis[time_indx_max],disp[time_indx_max],'x')
                        plt.plot(time_dis[time_indx_min],disp[time_indx_min],'o')
                        sadsa
                        
                        time_indx_max_old=time_indx_max
                        time_indx_min_old=time_indx_min
                        ...
                        # plt.plot(rbm)
                        # plt.plot(peaks_up,rbm[peaks_up],'o')
                        
                        # plt.plot(disp*30)
                        # plt.plot(time_indx_max,disp[time_indx_max]*30,'o')
                        # plt.plot(rbm)
                        # dsad
                        # else:
                            # dsad
                    ...
                    for rst in unique_ros_120:
                        ...
                        a1=np.array(tdms_file['Log'][rst_cmp[0]]).reshape((-1,1))#-np.mean(zero_tdms_file['Log'][rst_cmp[0]][:])).reshape((-1,1))
                        a2=np.array(tdms_file['Log'][rst_cmp[1]]).reshape((-1,1))#-np.mean(zero_tdms_file['Log'][rst_cmp[1]][:])).reshape((-1,1))
                        a3=np.array(tdms_file['Log'][rst_cmp[2]]).reshape((-1,1))#-np.mean(zero_tdms_file['Log'][rst_cmp[2]][:])).reshape((-1,1))
