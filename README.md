...
Python utilities, examples, and documentation for processing full-scale tidal
blade structural-test data. The public repository is designed to show the
analysis workflow without exposing private TDMS files, test logs, DIC images, or
...
This repository focuses on reusable analysis tasks from full-scale composite
tidal blade testing:
...
The main public scope is the analysis context of the first FastBlade / LoadTide
full-scale fatigue test and the later single-vs-multi-actuator comparison study.
Later clamping/load-introduction and destructive-testing studies are listed as
related FastBlade work, but this repository does **not** currently claim to
reproduce their DIC, clamping, or failure-analysis workflows.

## What this is not

This is not a replacement for the private experimental data archive. Raw TDMS
files, Excel logs, DIC images, and generated result folders are intentionally
ignored by Git. The repository provides a public software layer: code,
documentation, synthetic tests, configuration examples, and small public demos.
...
Synthetic examples are included so the workflow can be demonstrated without
publishing private experimental files.
...
The repository includes a lightweight applied-AI layer for long structural-test
time series. The purpose is to help prioritise engineering review, not to replace
it.

Included tools:

- fixed-window feature extraction for actuator loads, displacement, strain,
  root bending moment, pressure, or other processed channels;
...
The anomaly labels are **screening flags**, not confirmed damage labels. A
flagged window should be checked against the raw signal, test log, actuator
state, sensor health, environmental conditions, and engineering judgement.
...
These describe actuator count, actuator positions, target root bending moment,
loading direction, analysis outputs, and screening inputs without exposing
private data.
...
1. *A full-scale composite tidal blade fatigue test using single and multiple
   actuators*.

### Related FastBlade studies

1. *Clamping parameters in full-scale tidal turbine blade tests: A case study*.
1. *Destructive testing and failure analysis of a full-scale composite tidal
   turbine blade*.
...
A `CITATION.cff` file is included so GitHub can generate citation text for the
repository. The citation metadata also lists the primary and related FastBlade
publications.
